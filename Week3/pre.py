...
def select_rows(file_path, output_file_first, output_file_random):
    ...
    # Select the first 5000 rows and save to a new file
    df_first_5000 = df.head(5000)
    df_first_5000.to_csv(output_file_first, index=False)

    # Randomly select 5000 rows and save to another file
    df_random_5000 = df.sample(n=5000, random_state=42)
    df_random_5000.to_csv(output_file_random, index=False)

    print(f"First 5000 rows saved to: {output_file_first}")
    print(f"Random 5000 rows saved to: {output_file_random}")
...
if __name__ == "__main__":
    ...
    output_file_first_5000 = 'Dataset/absentee_first_5000.csv'
    output_file_random_5000 = 'Dataset/absentee_random_5000.csv'

    select_rows(file_path, output_file_first_5000, output_file_random_5000)   
